...
class Home_model extends CI_Model{   
    ...
    public function getPayment($sid=0) {
        ...
        $sql = "SELECT a.*,SUBSTRING_INDEX(SUBSTRING_INDEX(a.tanggal, ',', 2), ',', -1) as tanggal_new, b.jenis_pembayaran FROM t_payment as a JOIN t_pembayaran as b ON a.jenis_pembayaran = b.id_pay WHERE division_id='$dvs' and a.status not in ('99','XXX') AND tanggal2
    ...
    public function getDraftPrint(){
        ...
        $sql = "SELECT COUNT(status) as draftprint FROM t_payment WHERE status in ('1', '11') AND division_id='$dvs' AND tanggal2 BETWEEN '$start_date' AND '$end_date'";
    ...
    public function getPegawai(){
        ...
        $sql = "SELECT * FROM `m_user` WHERE division_id='$dvs' AND role_id <= 4";
        $query = $this->db->query($sql)->result();
        return $query;
    }
    ...
    function getARFPaid(){
		...
		$sql .="and a2.jenis_pembayaran=2 and a1.apf_doc not in (select a3.label5 FROM t_payment a3 WHERE a3.division_id ='$dvs' AND a3.status='1'  and a3.jenis_pembayaran=3) "; 
    ...
    public function getTotalDraft(){
        ...
        $sql = "SELECT COUNT(status) as totaldraft FROM t_payment WHERE division_id='$dvs' AND status in ('0','1','11')
                AND tanggal2 BETWEEN '$start_date' AND '$end_date'";
    ...
    public function getRejected(){
        ...
        $sql ="SELECT * FROM t_payment WHERE status='3' AND division_id='$dvs' ORDER BY rejected_date DESC";
	...
	function deletedsp3(){
        ...
        $sql = "SELECT a.*,SUBSTRING_INDEX(SUBSTRING_INDEX(a.tanggal, ',', 2), ',', -1) as tanggal_new, b.jenis_pembayaran FROM t_payment as a JOIN t_pembayaran as b 
                ON a.jenis_pembayaran = b.id_pay WHERE division_id='$dvs' and a.status in ('99','XXX') AND tanggal2
    ...
    public function buat_kode()  {   
        $dvs = $this->session->userdata('division_id');  
		$where = "division_id='".$dvs."' and substring(nomor_surat,length(nomor_surat)-1,2)='".date('y')."'";
    ...
    public function getDetailDraftPrint($sid=0,$start_date,$end_date) {
        ...
        $sql = "SELECT a.*, SUBSTRING_INDEX(SUBSTRING_INDEX(a.tanggal, ',', 2), ',', -1) as tanggal_new,b.jenis_pembayaran FROM t_payment as a JOIN t_pembayaran as b ON a.jenis_pembayaran = b.id_pay WHERE status in ('1','11') and division_id='$dvs'
                AND tanggal2 BETWEEN '$start_date' AND '$end_date'";
	...
	function getdetilarf($id){
        $dvs = $this->session->userdata('division_id');

        $sql = "select a2.label1,a2.currency,a2.currency2,a2.currency3,a2.label2,a2.jumlah2,a2.jumlah3 FROM t_payment_l a1, t_payment a2 WHERE a1.id_payment=a2.id_payment and a1.apf_doc='".$id."' and a1.division_id ='$dvs' AND a1.status='10' AND a1.apf_doc LIKE '%ARF%'";
		//var_dump($sql);
		//exit();
        $query = $this->db->query($sql)->result();
        return $query;
  ...
  public function getdatabyDR($profileid,$txtsearch)	{
  ...
  public function getdatabyLOP($profileid,$txtsearch){
