...
class Tri_model extends CI_Model{
    ...
    public function getpaidList() {
        ...
        $sql = "SELECT a.*, b.apf FROM t_payment_l as a JOIN t_pembayaran as b ON a.type = b.id_pay WHERE status='10' AND tanggal2 BETWEEN '$start_date' AND '$end_date'
                ORDER BY tanggal2 DESC";
    ...
    public function getVPayment() {
        $sql = "SELECT a.tanggal2, b.jenis_pembayaran, COUNT(a.type) as jmlpembayaran FROM t_payment_l a RIGHT JOIN t_pembayaran b ON a.type = b.id_pay 
                WHERE b.jenis_pembayaran != '' AND a.type != 0 AND a.status in ('9','10') GROUP BY b.jenis_pembayaran";
    ...
    public function getVPaymentPeriode($start_date,$end_date) {
        ...
        $sql = "SELECT a.tanggal2, b.jenis_pembayaran, COUNT(a.type) as jmlpembayaran FROM t_payment_l a RIGHT JOIN t_pembayaran b ON a.type = b.id_pay 
                WHERE b.jenis_pembayaran != '' AND a.type != 0 AND a.status in ('9','10') AND a.tanggal2 BETWEEN '$start_date' AND '$end_date' GROUP BY b.jenis_pembayaran";
